...
describe("marketsActiveIn with tenures", () => {
  ...
  it("takes tier from the tenure row, so a market can change tier by year", () => {
    expect(
      marketsActiveIn(1967, data.markets, data.territories, tenures).find((m) => m.id === "city1")
        ?.tier,
    ).toBe("Primary");
    expect(
      marketsActiveIn(1975, data.markets, data.territories, tenures).find((m) => m.id === "city1")
        ?.tier,
    ).toBe("Secondary");
  });
  ...
  it("drops a tenured market in years no tenure covers", () => {
    ...
    expect(
      marketsActiveIn(1990, data.markets, data.territories, tenures).some((m) => m.id === "city1"),
    ).toBe(false);
  });
  ...
  it("honours a tenure before its territory is formally constituted", () => {
    ...
    const early = [{ marketId: "city1", territoryId: "b", fromYear: 1965, toYear: 1969 }];
